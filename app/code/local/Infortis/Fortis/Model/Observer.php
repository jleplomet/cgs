...
class Infortis_Fortis_Model_Observer
{
	/**
	...
	public function hookTo_controllerActionPostdispatchAdminhtmlSystemConfigSave()
	{
		...
		if ($section == 'fortis_layout')
		{
			$websiteCode = Mage::app()->getRequest()->getParam('website');
			$storeCode = Mage::app()->getRequest()->getParam('store');

			$cg = Mage::getSingleton('fortis/cssgen_generator');
			$cg->generateCss('grid',   $websiteCode, $storeCode);
			$cg->generateCss('layout', $websiteCode, $storeCode);
		}
		elseif ($section == 'fortis_design')
		{
			$websiteCode = Mage::app()->getRequest()->getParam('website');
			$storeCode = Mage::app()->getRequest()->getParam('store');
			
			Mage::getSingleton('fortis/cssgen_generator')->generateCss('design', $websiteCode, $storeCode);
		}
		elseif ($section == 'fortis')
		{
			$websiteCode = Mage::app()->getRequest()->getParam('website');
			$storeCode = Mage::app()->getRequest()->getParam('store');
			
			Mage::getSingleton('fortis/cssgen_generator')->generateCss('layout', $websiteCode, $storeCode);
		}
	}
